import { associatedQuestions } from '@/database/local-date'
...
const QuizBox: any = () => {
  ...
  const { useStorageResponseFetcher } = useStorageResponse(quizData, 1)
  ...
  const HandlerResult = (result: number) => {
    ...
    sessionStorage.setItem('associated', '1')
    setTimeout(() => {
      router.push('/result')
    }, 2500)
  };
  ...
  useEffect(() => {
    const isResult = stepCount === associatedQuestions.qsns.length 
    if(isResult) {
  ...
  const processData = (responseValue: number, qsn: string, responseString: string) => {
    ...
    setResult(prev => prev + (responseValue + 1))
  ...
  const handlerAnswer = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const questionData = associatedQuestions.qsns[questionIndex]
    const responseString = e.target.value
    const responseValue = associatedQuestions.qsns[questionIndex].optn.indexOf(responseString)
  ...
  const Quiz = ({ qsno }: { qsno: number }) => (
    qsno >= 0 && qsno < associatedQuestions.qsns.length ? (
      <>
        <h4 
          className="mb-8 text-heading-6 font-bold text-dark dark:text-white"
        >
              {associatedQuestions.qsns[qsno].qsn}
        </h4>
        <div className="flex flex-col space-y-4 p-4">
          {associatedQuestions.qsns[qsno].optn.map(ans => (
            <>
              <button
                  // @ts-ignore
  ...
  const isQuestion = (stepCount !== associatedQuestions.qsns.length && stepCount !== -1) 
  const isResult = stepCount === associatedQuestions.qsns.length 
